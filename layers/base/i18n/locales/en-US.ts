import { zhFallbackLocale } from "./merge";

export default defineI18nLocale(() =>
  zhFallbackLocale({
    share: {
      title: "Share to Friends / Moments",
      openingTip: "① Tap ··· in the top right　② Choose \"Share to Chat / Moments\"",
      inviteTip: "Your invite link includes your code — friends who sign up and order get linked to you",
      copyLink: "Copy Link",
      copied: "Copied",
      copyTalk: "Copy Invite Message",
      talkPrefix: "Come browse with me, there are awesome deals → ",
      inviteCount: "{n} invited",
      wechatLogin: "Login with WeChat",
      inviteBar: "Log in with WeChat to get exclusive perks & bind your inviter",
      inviteBarClose: "Not now",
      loginSuccess: "Signed in",
      loginFail: "Sign-in failed",
      callbackFail: "Login callback failed. Please try again.",
      callbackProcessing: "Completing sign-in and redirecting…",
    },
    site: {
      title: "youshop.cn",
      shareDesc: "youshop.cn一站式专家购物，跨境优选，私人订制。",
      tagline: "Nuxt Level Headless E-commerce",
      shortDescription:
        "A modern, fast, and secure foundation for building headless online stores with Nuxt and Vendure.",
      description:
        "Nuxtless is a flexible and modular starter project for headless e-commerce, designed for high performance, excellent SEO, and clean architecture. It offers ready integrations with Vendure, Nuxt UI, and Payload, type-safe code, and professional security practices — a solid foundation for any professional online store.",
    },
    nav: {
      back: "Back",
      home: "Home",
      categories: "Categories",
      allCategories: "All Categories",
      allProducts: "All Products",
      my: "My",
      cart: "Cart",
      search: "Search",
      searchPlaceholder: "Search...",
      loading: "Loading…",
      selectCity: "Select City",
      locating: "Locating…",
      hotCities: "Hot Cities",
      noCityData: "No city data for this region",
      more: "More ›",
      brandFlash: "Brand Flash Sale",
      reloadLocation: "Re-locate",
      backToProvince: "Back to Provinces",
      allProvinces: "All Provinces",
      viewMore: "View More",
      categoryNav: "Category Navigation",
      bottomNav: "Bottom Navigation",
      previous: "Previous",
      next: "Next",
      noMatchProducts: "No matching products",
      qualityZone: "Quality Zone",
    },
    home: {
      newsTitle: "JD News",
      adsAlt: "Promo ad",
      mail: "Mail",
      selfPickup: "Self Pickup",
      emptyAfterFilter: "No products available for your city / delivery method",
      news: [
        "Free shipping on all self-operated items over ¥99",
        "¥20 off your first order for new customers",
        "Trade-in for digital and home appliances",
        "Flash sale today from 20:00",
        "7-day no-reason returns",
      ],
      entryLabels: [
        "Mobile & Digital",
        "Home Appliances",
        "Household Goods",
        "Apparel & Bags",
        "Beauty & Care",
        "Food & Fresh",
        "Sports & Outdoors",
        "Custom Gifts",
        "All Products",
      ],
    },
    detail: {
      marketingTags: { new: 'New', hot: 'Hot', special: 'Special', sale: 'Flash Sale', freeShip: 'Free Ship', cut: 'Discount', clearance: 'Clearance', instock: 'In Stock', 'flash-sale': 'Flash Sale', limited: 'Limited', 'price-drop': 'Price Drop', 'new-arrival': 'New Arrival' },
      reviews: 'Reviews',
      reviewsEmpty: 'No reviews yet. Be the first to review this product.',
      serviceItems: ['Genuine Products', 'Fast Dispatch', 'After-sales Support'],
      promoItems: ['7-day no-reason returns', 'Free shipping over ¥99'],
      buyNow: 'Buy Now',
      addToCart: 'Add to Cart',
      promoSummary: 'Promotions',
      serviceSummary: 'Services',
      couponTitle: 'Exclusive product coupons',
      couponClaim: 'Claim',
      couponClaimed: 'Claimed',
      couponClaimSuccess: 'Claimed successfully',
      couponClaimFailed: 'Claim failed',
      couponNewCustomer: 'New customers only',
      couponNoThreshold: 'No minimum',
      couponMinSpend: 'Use on orders over {n}',
      couponExpires: 'Valid until {date}',
      couponValidDays: 'Valid for {n} days after claim',
      couponFull: 'Free item',
      couponFreeShipping: 'Free shipping',
      inStock: 'In Stock',
      outOfStock: 'Out of Stock',
      floorDescription: 'Details',
      floorSpecs: 'Specs',
      floorReviews: 'Reviews',
      floorService: 'Service',
      notServiceable: 'This product is not deliverable to your current city',
      viewServiceCities: 'View deliverable cities',
      serviceCitiesTitle: 'Deliverable cities',
      nationwide: 'Nationwide delivery',
      nearbyLoading: 'Checking nearby stock…',
      nearbyTitle: 'Nearby Stock',
      nearbyNoCoords: 'Enable location to view nearby stock',
      nearbyError: 'Nearby stock temporarily unavailable',
      nearbyNoStock: 'No nearby store stock available',
      nearbySummary: 'Nearby stock: {qty} available',
      stockCount: 'Stock: {n}',
      deliveryMail: 'Mail',
      deliveryPickup: 'Self Pickup',
      deliveryNoStock: 'In-store pickup; stock confirmed at store',
      pickupSummary: 'Available at nearby pickup points: {qty}',
      nearbyStoresCount: '{n} stores · nearest ~{d}',
      nearbyUnknownDistance: 'distance unknown',
      addedToCart: 'Added to cart',
      buyNowFailed: 'Purchase failed, please retry',
      stockShortage: 'Insufficient stock, added {n} item(s)',
      sku: 'SKU: {code}',
      spec: 'Spec: {name}',
      specDefault: 'Default',
      checkIn: 'Check-in',
      checkOut: 'Check-out',
      nights: '{n} nights',
      nightsRange: '{min}-{max} nights required',
      perNight: '/night',
      room: 'Room',
      select: 'Select',
      selected: 'Selected',
      expandRooms: 'Room details ({n})',
      collapseRooms: 'Collapse rooms',
      breakfastIncluded: '{n} breakfasts included',
      breakfastNotIncluded: 'No breakfast',
      addBed: 'Extra bed available',
      checkInOut: 'Check-in / Check-out',
      cancelPolicy: 'Cancellation',
      deposit: 'Deposit',
      freeUntil: 'Free cancel {h}h before check-in',
      nonRefundable: 'Non-refundable',
      payAtHotel: 'Pay at hotel',
      prepay: 'Prepay',
      noDeposit: 'No deposit',
      addBedFee: 'Extra bed fee',
      priceByDay: 'Daily pricing',
      total: 'Estimated total',
      avgNight: 'Avg / night',
      tWeekday: 'Weekday',
      tWeekend: 'Weekend',
      tHoliday: 'Holiday',
      tCustom: 'Special',
      bedType: 'Bed',
    },
    pages: {
      index: {
        welcome: "Welcome to",
      },
      account: {
        signIn: "Sign in to Your Account",
        requestPasswordReset: "Request Password Reset",
        accountVerify: "Account Verification",
      },
    },
    coupon: {
      title: 'Coupons',
      couponCentre: 'Coupon Centre',
      myWallet: 'My Coupons',
      noAvailable: 'No coupons available to claim',
      loginPrompt: 'Please log in to claim coupons / view your wallet',
      loginPromptOrder: 'Log in to use your wallet coupons',
      goLogin: 'Log In',
      claim: 'Claim Now',
      claimed: 'Claimed',
      soldOut: 'Sold Out',
      claimSuccess: 'Coupon claimed',
      claimFailed: 'Claim failed',
      perUserReached: 'Claimed',
      unused: 'Unused',
      used: 'Used',
      expired: 'Expired',
      returned: 'Returned',
      emptyUnused: 'No available coupons',
      emptyUsed: 'No used coupons',
      emptyExpired: 'No expired coupons',
      emptyReturned: 'No returned coupons',
      expiringDays: '{n} days left',
      code: 'Code',
      usedStamp: 'Used',
      expiredStamp: 'Expired',
      returnedStamp: 'Returned',
      voucher: 'Coupon',
      validity: 'Validity',
      dateRange: '{start} – {end}',
      dateUntil: 'until {end}',
      typeFixed: 'Instant Discount',
      typePercent: 'Discount',
      typeFull: 'Flat Discount',
      typeFreeShipping: 'Free Shipping',
      unitYuan: '¥',
      unitDiscount: 'OFF',
      noThreshold: 'No minimum',
      noThresholdFull: 'No minimum flat discount',
      minSpend: 'On orders over {n}',
      coupon: 'Coupons',
      chooseCoupon: 'Choose a coupon',
      viewWallet: 'View wallet',
      currentlyApplied: 'Applied to your order',
      apply: 'Apply',
      applied: 'Applied',
      appliedError: 'Apply failed',
      remove: 'Remove',
      removed: 'Removed',
      noUsableCoupon: 'No usable coupons',
      couponUnavailable: 'Coupon unavailable',
      couponUsed: 'Coupon applied',
      couponCleared: 'Coupon removed',
      noCoupon: 'No coupon',
      redeemTitle: 'Redeem with a code',
      redeemPlaceholder: 'Enter redeem code',
      redeemBtn: 'Redeem',
      redeemSuccess: 'Redeemed successfully',
      redeemFailed: 'Redemption failed',
      drawerUnavailableAll: 'You have {n} coupon(s) not usable at this store',
      drawerUnavailableReasons: '{m} below threshold · {k} not for this store',
    },
    general: {
      recommendations: 'Recommended for you',
      cancel: 'Cancel',
      save: 'Save',
      colorMode: "Color Mode",
      system: "System",
      light: "Light",
      dark: "Dark",
      actions: "Actions",
      details: "Details",
      getLink: "Copy Link",
      getLinkSuccess: "Order link copied to clipboard!",
      back: "Back",
      citySearchHint: "Start typing to search cities…",
      date: "Date",
      status: "Status",
      amount: "Amount",
      paymentMethod: "Payment Method",
      loginRequiredForPayment: "Please log in to use this payment method",
      shipping: "Shipping",
      shippingDetails: "Shipping Details",
      shippingSelect: "Shipping Method",
      shippingAddress: "Delivery Address",
      tax: "Tax",
      home: "Home",
      product: "Product",
      menu: "Menu",
      mobileMenu: "Mobile Menu",
      apply: "Apply",
      loading: "Please wait...",
      to: "to",
      and: "and",
      printReceipt: "Print Receipt",
      generalMessage: "Please try again later or contact support.",
      shopFeatures: {
        shipping: "Free shipping",
        shippingText: "Enjoy free delivery on all orders over €50.",
        returns: "30-day returns",
        returnsText: "Not satisfied? Return your order within 30 days.",
        checkout: "Secure checkout",
        checkoutText: "Powered by Stripe for encrypted, safe payments.",
        support: "Always here",
        supportText: "24/7 online support for all your questions.",
      },
      footer: {
        unstack: "Built with ❤️ on Unstack",
      },
    },
    checkout: {
      deliveryMethod: "Delivery Method",
      pickupMethod: "Pickup Method",
      logisticsDelivery: "Logistics Delivery",
      deliveryLogistic: "Delivery",
      chooseCarrier: "Select a carrier",
      pickupFillFromLocation: "Pickup selected: showing pickup points and hiding the delivery address",
      storePickup: "Store Pickup",
      employeePickup: "Employee Pickup",
      pointPickup: "Pickup Point",
      searchPickupPlaceholder: "Search pickup point by name or address",
      boxGroupTitle: "Delivery group {n}",
      boxLogisticsOption: "Delivery methods for this group",
      boxPickupOption: "Pickup points for this group",
      needBoxDelivery: "Please select a delivery or pickup option for each delivery group",
      navigation: "Direction",
      goNavigate: "Navigate",
      navNoCoords: "No usable navigation coordinates for this pickup point",
      navSdkFail: "Failed to load the map, please try again later",
      deliveryTo: "Deliver to",
      deliveryToDesc: "Name · Phone · Region · Street",
      addAddress: "Add Address",
      switchAddress: "Switch Address",
      saveAddress: "Save Address",
      needAddress: "Please fill in a delivery address",
      useBook: "Address book",
      invalidAddress: "Incomplete delivery address",
      choosePickup: "Choose a pickup location",
      nearestHint: "Nearest preselected, you can change it",
      locateHint: "Enable location to preselect the nearest pickup",
      noPickup: "No pickup location nearby for now, try another option",
      needPickup: "Please choose a pickup location first",
      noShippingMethod: "No delivery method available",
      noShippingMethodDesc: "No delivery method is available for this address. Try store/pickup point, or change the address.",
      pickupContactTitle: "Pickup Contact",
      pickupContactName: "Contact name",
      pickupContactPhone: "Phone",
      pickupRemark: "Remark (optional)",
      pickupContactRequired: "Contact name & phone are required",
      invalidPhone: "Invalid phone number",
      newContact: "Add new contact",
      pickContactPerson: "Select contact",
      switchContact: "Switch / add contact",
      locateAddressDone: "Shipping address filled from location",
      orderBoxesLoading: "Shipping info is loading, please try again in a moment",
      completeSections: "Please complete shipping and payment info first",
      pickPaymentMethod: "Please select a payment method",
      cnContactTitle: "Contact Information",
      cnAgreementTerms: "User Service Agreement",
      cnAgreementPrivacy: "Privacy Policy",
      cnAgreementNotice: "I have read and agree to",
      cnAgreementRequired: "Please agree to the terms and privacy policy before submitting",
      cnExpandDetails: "Details",
      cnCollapseDetails: "Collapse",
      settlementTitle: "Settlement",
      merchantSplit: "Merchant Split",
      payableTotal: "Total Payable",
      mergeToOneOrder: "Will be paid as 1 order",
      splitPay: "Boxes use different payment methods — they will be paid separately",
      splitPerBox: "Order {n}",
      perBoxSubtotal: "This box amount",
      uniqueMethodLocked: "Only method for this box · Auto-locked",
      noAvailablePaymentMethod: "No available payment method",
      emptySelection: "Please select at least one item",
      flowBack: "{n} item(s) not checked out, returned to cart",
      flowBackTitle: "Returned to cart",
      shipping: "Shipping",
      subtotal: "Subtotal",
      couponDiscount: "Coupon / Free-shipping",
      selectedCount: "Selected {n} item(s)",
    },
    account: {
      messages: 'Messages',
      noMessages: 'No messages yet',
      password: "Password",
      newPassword: "New Password",
      confirmNewPassword: "Confirm New Password",
      updatePassword: "Update Password",
      otherLogin: "Other login methods",
      forgotPassword: "Forgot Password?",
      rememberPassword: "Remembered your password?",
      passwordPlaceholder: "Enter your password",
      firstName: "First Name",
      firstNamePlaceholder: "Enter your first name",
      lastName: "Last Name",
      lastNamePlaceholder: "Enter your last name",
      enterPassword: "Enter Password",
      confirmPassword: "Confirm your password",
      email: "Email",
      emailPlaceholder: "Enter your email",
      login: "Log In",
      logout: "Log Out",
      backToLogin: "Back to login.",
      ifRegistered:
        "If your email address is registered with us, you will receive an email with a link to reset your password.",
      resetPassword: "Reset Password",
      sendPasswordResetEmail: "Send Email",
      register: "Register",
      accountRegister: "Sign Up",
      noAccount: "Don't have an account?",
      hasAccount: "Already have an account?",
      myAccount: "My Account",
      profile: "Profile",
      myPhone: "My Phone",
      myAddress: "My Address",
      orders: "My Orders",
      loginToAccount: "Log in to Your Account",
      loginSuccess: "Login Successful",
      successMessage: "You logged in into your account.",
      loginFail: "Login Failed",
      failMessage: "The provided credentials are invalid.",
      registerSuccess: "Registration Successful",
      registerSuccessMessage: "Please check your email to verify your account.",
      registerFail: "Registration Failed",
      verifySuccess: "Account Verified",
      verifySuccessMessage: "You can now login with your email and password.",
      resetSuccess: "Password Reset",
      resetMessage: "Your password has been reset.",
      addresses: "Addresses",
      addAddress: "Add Address",
      edit: "Edit",
      delete: "Delete",
      defaultAddress: "Default",
      noAddresses: "No addresses yet",
      selectAddress: "Select from address book",
      saveSuccess: "Saved",
      deleteSuccess: "Deleted",
      backToAccount: "Back to my account",
      contactName: "Contact name",
      phone: "Phone",
      afterSales: "After-Sales",
      noDefaultAddress: "No default address",
      profileInfo: "Profile Information",
      phoneLabel: "Phone",
      addressLabel: "Address",
      accountActions: "Account Actions",
      verifyingAccount: "Verifying your account...",
      verificationComplete: "Verification complete. Redirecting…",
      selfOperated: "Self-operated",
      coupons: "My Coupons",
      couponCentre: "Coupon Centre",
    },
    afterSales: {
      title: "After-Sales",
      detailTitle: "After-Sales Detail",
      backToList: "Back to after-sales",
      apply: "Apply",
      applyTitle: "Apply After-Sales",
      type: "Type",
      typeReturnRefund: "Return & Refund",
      typeRefundOnly: "Refund Only",
      typeExchange: "Exchange",
      typeUnknown: "After-Sales",
      refundAmount: "Refund Amount",
      amountHint: "max",
      amount: "Refund Amount",
      reason: "Reason",
      reasonPlaceholder: "Describe the reason",
      reasonHint: "Provide a reason and amount within limit",
      description: "Description",
      descPlaceholder: "Optional details",
      submit: "Submit",
      submitTracking: "Submit",
      cancel: "Cancel",
      cancelSuccess: "Request cancelled",
      createSuccess: "Request submitted",
      trackingSuccess: "Tracking updated",
      trackingNo: "Return Tracking No.",
      carrier: "Carrier",
      carrierPlaceholder: "e.g. ZTO Express",
      trackingPlaceholder: "Tracking number",
      trackTitle: "Enter Return Tracking",
      orderCode: "Order",
      rejectReason: "Reject Reason",
      empty: "No after-sales yet",
      notFound: "Not found",
      tabAll: "All",
      tabPending: "Pending",
      tabToReturn: "To Return",
      tabReturning: "In Progress",
      tabRefunded: "Refunded",
      tabRejected: "Rejected",
      tabClosed: "Closed",
      statePending: "Pending Review",
      stateApproved: "Approved",
      stateRejected: "Rejected",
      stateReturning: "Returning",
      stateReceived: "Received",
      stateRefunded: "Refunded",
      stateClosed: "Closed",
      stateUnknown: "Processing",
      stepPending: "Apply",
      stepApproved: "Approved",
      stepReturning: "Returning",
      stepReceived: "Received",
      stepRefunded: "Refunded",
      evidence: "Evidence",
      noEvidence: "No evidence uploaded",
      viewEvidence: "View evidence",
      createdAt: "Requested At",
      updatedAt: "Last Updated",
      hours: "Service Hours",
      customerService: "Contact Support",
      customerServiceHint: "For after-sales updates or assistance, reach us via:",
      wechat: "WeChat",
      wechatCopied: "WeChat ID copied, add us on WeChat",
      cancelConfirm: "Cancel this after-sales request?",
      cancelConfirmDesc: "This action cannot be undone. Please confirm.",
      keepRequest: "Keep Request",
      confirmCancel: "Confirm Cancel",
      refresh: "Refresh",
      loadFailed: "Failed to load, please check your network",
      retry: "Retry",
    },
    shop: {
      category: "Category | Categories",
      shopByCategory: "Shop by Category",
      popularProducts: "Popular Products",
      inStock: "In Stock",
      lowStock: "Low Stock",
      outOfStock: "Out of Stock",
      price: "Price",
      priceFrom: "From",
      priceSave: "Save",
      priceFlash: "Price drop",
      priceFinal: "Final price",
      priceFlashSale: "Flash sale",
      addToCart: "Add to Cart",
      close: "Close",
      rateEmail: "Your email",
      orderSummary: "Order Summary",
      orderReceived: "Order Received",
      orderCode: "Order Code",
      orderDetails: "Order Details",
      orderThanks:
        "Thank you for your order! Please check your email for further information and don't forget to check your spam folder. If the email you provided is incorrect or you do not receive a confirmation within 5–10 minutes, please contact our support team and quote your order code. Standard delivery takes around 3–5 business days. We truly appreciate your trust in us and look forward to serving you again.",
      thankYou: "Thank you!",
      total: "Total",
      subtotal: "Subtotal",
      yourCart: "Your Cart",
      cartDescription: "Items currently in your cart",
      cartEmpty: "Your cart is empty.",
      browseProducts: "Browse Products",
      browseOurProducts: "Browse our products",
      quantity: "Quantity",
      checkout: "Checkout",
      searchProducts: "Search Products",
      searchPlaceholder: "Type to search...",
      searchLoading: "Loading",
      searchProductsAria: "Search products",
      couponCode: "Coupon Code",
      noProductsFound: {
        title: "No products found",
      },
      filters: "Filters",
      noFilters: "No filters available",
      clearFilters: "Clear Filters",
      applyFilters: "Apply Filters",
      outOfArea: "Out of Area",
      pickupInfo: "Pickup / Verification Info",
      pickupCode: "Pickup Code",
      pickupClaimed: "Claimed",
      pickupPending: "Pending",
      packageShipping: "Package Shipping",
      warehouse: "WH",
      shippingAdjustmentCharge: "Extra Charge",
      shippingAdjustmentRefund: "Refund",
      contactPhone: "Contact",
      orderSuccessful: "Order Successful",
      thankYouForOrder: "Thank you for your order.",
    },
    order: {
      tabAll: "All",
      tabPaymentPending: "Payment Pending",
      tabToShip: "To Ship",
      tabToReceive: "To Receive",
      tabCompleted: "Completed",
      tabCancelled: "Cancelled",
      statePaymentPending: "Payment Pending",
      statePaid: "Paid",
      stateShipped: "Shipped",
      stateDelivered: "Completed",
      stateCancelled: "Cancelled",
      stateProcessing: "Processing",
      progressPlaced: "Placed",
      progressPaid: "Paid",
      progressShipped: "Shipped",
      progressCompleted: "Completed",
      delivery: "Delivery",
      expressNo: "Tracking No.",
      expressCompany: "Carrier",
      pickupType: "Pickup Type",
      cancel: "Cancel Order",
      cancelNotAllowed: "Paid/shipped orders require support assistance",
      cancelSuccess: "Order cancelled",
      cancelFailed: "Failed to cancel",
      reorder: "Reorder",
      reorderSuccess: "Added to cart",
      reorderFailed: "Failed to add to cart",
      noAddress: "No shipping address",
      // Guest order lookup
      lookupTitle: 'Look up order',
      lookupMenu: 'Look up order',
      lookupSubmit: 'Search',
      lookupNotFoundClass: 'No matching order found. Check the phone number and order code',
      orderCodeLabel: 'Order code',
      phoneLabel: 'Phone number',
      orderCodePlaceholder: 'Enter order code',
      phonePlaceholder: 'Enter phone number',
      totalWithTax: 'Amount paid',
      pickupKeepHint: 'Keep this order code and pickup code to present at the store',
      addPhoneTitle: 'Add phone number',
      addPhoneHint: 'Add your phone number to look up the order anytime with phone + order code',
      savePhone: 'Save',
      phoneSaved: 'Phone saved. You can look up the order anytime with phone and order code',
      orderItems: 'Items',
      viewDetail: 'View',
      empty: 'No orders yet',
      loadMore: 'Load more',
      loadFailed: 'Failed to load, try again later',
      actualPaid: 'Paid',
      totalItems: '{n} item(s)',
      redemptionTitle: 'Redemption',
      redemptionCodeLabel: 'Code',
      redemptionUnavailable: 'No redemption data found',
      redeemed: 'Redeemed',
      redeemPending: 'Pending',
      redeemSuccess: 'Redeemed successfully',
      redeemAlready: 'This code has already been redeemed',
      redeemNotFound: 'No order found for this redemption code',
      redemptionAdminTitle: 'Order Redemption',
      redemptionAdminSub: 'Enter or scan the 6-digit redemption code (case-insensitive, auto-uppercased) to look up and redeem an order.',
      redemptionCodePlaceholder: 'Enter / scan the 6-digit redemption code',
      redemptionLookupBtn: 'Look up',
      redeemConfirm: 'Confirm redemption',
      redeemStatusPending: 'Pending pickup',
      redeemStatusExpiring: 'Expiring soon',
      redeemStatusExpired: 'Expired',
      redeemStatusReissued: 'Reissued · active now',
      redeemExpiresTo: 'Valid until {date}',
      redeemExpireSoonTip: 'Code expires soon, please pick up in time',
      redeemExpiredTip: 'Code expired, please contact the pickup point',
      redeemReissuedTip: 'Previous code voided, use the new code below',
      redeemReissueBtn: 'Reissue',
      redeemReissueSuccess: 'Redemption code reissued',
      redeemReissueFailed: 'Reissue failed',
      redeemAlreadyClaimed: 'Already redeemed, cannot reissue',
      redeemPoint: 'Pickup point',
      adminTokenLabel: 'Admin Token',
      adminTokenNote: 'The full Admin login (separate login page / route guard) is future work; the token is kept in memory/sessionStorage only.',
      adminTokenSet: 'Token held (current session)',
      adminTokenNotSet: 'No token set',
      adminUsername: 'Admin account',
      adminPassword: 'Password',
      getToken: 'Get token',
      clearToken: 'Clear',
      pasteToken: 'Or paste the Admin token directly',
      saveToken: 'Save token',
      needTokenFirst: 'Please enter/get an admin token first (see the "Admin Token" section below)',
      orderCodeAdmin: 'Order code',
      redemptionStatus: 'Redemption status',
      collectPending: 'Awaiting in-store payment',
      collected: 'Collected',
      paymentMethods: {
        'balance-wallet': 'Wallet Balance',
        'stripe-payment': 'Stripe',
        stripe: 'Stripe',
        alipay: 'Alipay',
        wechat: 'WeChat Pay',
        'wechat-pay': 'WeChat Pay',
        paypal: 'PayPal',
        'cash-on-delivery': 'Cash on Delivery',
        cod: 'Cash on Delivery',
        'cod-payment-template': 'Pay at Store',
        'cloud-payment-template': 'Pay at Store',
      },
      claimedAt: 'Redeemed at',
      quantityAmount: 'Items / Amount',
      unit: 'item(s)',
      confirmRedeem: 'Confirm redemption',
      nextOne: 'Next order',
      queryLoading: 'Looking up…',
      claimLoading: 'Redeeming…',
      opFailed: 'Operation failed',
      ok: 'OK',
      tokenAcquired: 'Token acquired (kept for this session only)',
      noTokenReturned: 'Login returned no token, please paste it manually',
      netError: 'Network error',
      nonJsonResponse: 'Non-JSON response (HTTP {status})',
      graphqlError: 'GraphQL error',
      merchantPickup: 'Pickup store',
    },
    billing: {
      recipient: "Recipient",
      recipientPlaceholder: "Enter the recipient name",
      address1: "Detailed Address",
      address2: "Address 2 (building, floor, apartment)",
      company: "Company (optional)",
      province: "Province",
      city: "City",
      district: "District",
      street: "Street / Township",
      zip: "Postal Code / ZIP",
      country: "Country",
      phoneNumber: "Phone Number",
      phoneNumberPlaceholder: "Enter your phone number",
      email: "Email (optional)",
      isDefault: "Set as default shipping address",
      loadingDistricts: "Loading regions…",
      requiredFullName: "Please enter the recipient name",
      requiredPhone: "Please enter your phone number",
      requiredAddress: "Please enter the detailed address",
      requiredAddress2: "Please enter Address 2 (building, floor, apartment)",
      requiredPostalCode: "Please enter the postal code",
      requiredCity: "Please select a city",
      requiredCountry: "Please select a country",
      invalidEmail: "Invalid email address",
      address1Placeholder: "Detailed address (street, number)",
    },
    error: {
      invalidPasswordResetLink: "Password reset link is invalid.",
      resetFail: "Reset failed",
      general: "Something went wrong",
      generalMessage: "Please try again later or contact support.",
      noOrder: "Order not found",
      orderNotFound:
        "We couldn't find your order. Please log in to view your orders.",
      orderExpiredLink:
        "This order does not exist or the link has expired. Please check the order number, verify via \"Order Lookup\", or contact support.",
      guestOrderTitle: "Order Confirmation",
      guestOrderHint:
        "Here is the order you looked up. Please verify the pickup code to redeem it at the store.",
      guestNotFound:
        "No matching order found. It may have expired or the link is incorrect. Please verify via \"Order Lookup\".",
    },
  }),
);
