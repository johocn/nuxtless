...
## 总结报告（2026-09-19 · 5 级可回退风格体系手动测试）

### 一、测试范围与目标

针对 nshop 全店体系（C 端 www.youshop.cn + web-admin 运营后台 e.joho.cn/guanli）的 **5 级可回退风格体系**做全量线上验证：用「制造上级差异 → 逐级撤销 → 验证回退」的方法，证明每一级的配置是否真实生效、回退是否兜底、是否会被上一级正确覆盖。同时也校验了后台相关 UI（店铺覆盖模板按目标端过滤）。

### 二、测试了哪些功能

| 阶段 | 功能点 | 说明 |
|---|---|---|
| **Part A 分层矩阵** | 逐层验证 L0–L4 有效性 | L0 代码内建默认 / L1 全局配置 / L2 风格模板 / L3 店铺覆盖 / L4 块内建默认 |
| A1 | 极端清空回退 | 清光 L1 defaults + 渠道 templateId → 纯代码默认（橙 + classic + 全块可见 + 不空白）|
| A2/A3 | L1 页面级生效 + L2 覆盖与回退 | 无模板时 L1 生效；引用模板覆盖 L1；停用模板回退 L1 |
| A4 | L3 店铺覆盖 | channel `detailConfig` 覆盖模板版式（classic→floor），未覆盖字段保持 L2 |
| A5 | L4 块内建默认 | 单块 `visible:false` 只隐藏该块，其余块回退内建默认 true |
| **Part B 端到端** | 五类真实运营场景 | B1 全店换肤 / B2 切换模板 / B3 单店覆盖 / B4 模板停用回退 / B5 新店默认 L1 |
| B1 | 全店换肤 | 模板 palette 改主色 → 全站联动（首页/详情/购物车/我的）|
| B2 | 切换模板 | templateId 切换 → 全站配色联动 |
| B3 | 单店覆盖 | detailConfig dualBuy + promo 隐藏，只影响声明字段 |
| B4 | 模板停用回退 | 停用/删除/跨端 → 回退 L1，不报错不空白 |
| B5 | 新店默认 | 未引用模板的渠道 → 直接落 L1 开箱即用 |
| **P2 (UI)** | 后台店铺覆盖模板按目标端过滤 | nshop/vshop 目标端分段器双向过滤，互不可见对方模板 |

### 三、测试结果

**9 项（A1–A4、B1–B5）、全部线上实测通过**，均为浏览器手机视口（390×844）截图判定。结论：

- **五级回退链端到端成立**：L0 是最底层稳定兜底（极端清空仍正常渲染）；L1/L2/L3 逐级正确覆盖上一级；L4 块级隔离、缺省回退内建默认。
- **回退不空白**：引用模板停用/删除/跨端时后端 `shopTemplate` 返回 null，C 端稳定回退 L1，无报错、无空白页。
- **覆盖只影响声明字段**：L3 单店覆盖不污染模板/全局/其他渠道。
- **P2 后台过滤有效**：nshop / vshop 模板列表按目标端双向隔离，避免跨端误选无效模板。

### 四、发现的问题与改进建议

| # | 现象 | 结论/建议 |
|---|---|---|
| **P5** | 改配置后 C 端首帧仍是旧配置 | 缓存滞后（nginx/SSR）。**建议**：改模板/渠道后强刷或 `?cb=` 冷加载确认；判定一律以冷加载为准 |
| **P6** | 引用模板时改 L1 全局 `themeTokens` 主色不生效 | **设计语义，非 bug**——主题令牌优先级 L1 < L2 模板 palette。**建议**：引用模板的店全店换肤入口 = 编辑模板 `palette.tokens`；团队文档标注清楚，避免误判为失效 |
| **P3** | vshop 模板（id=4/5/6）零渠道引用 | 属预留。不建议强引；若启用须走有效模板（跨端会自动回退 L1）|
| — | **判据纪律** | curl/SSR 对某些动态块判定不可靠（曾对促销块误判为空商品）。**建议**：C 端视觉结果一律以浏览器手机视口截图为准，SSR/shop-api 仅作辅助 |

**总体评价**：5 级可回退风格体系功能完整、回退兜底可靠，是一套可安全交给运营使用的风格配置底座。主要改进方向集中在「缓存可见性」与「团队认知标注」两项，而非渲染链路本身。
...
| 交付物 | 位置 |
|---|---|
| ① 手动测试文档（本文件） | `docs/superpowers/manual/5level-fallback/5level-fallback-manual-test.md` |
| ② 后台设计说明（spec 第 6 章） | `docs/superpowers/specs/2026-09-19-5level-fallback-test-design.md` |
| ③ C 端预计效果 mockup | 会话内联可视化（A/B/C 版式方案） |
| ④ 问题清单 + 本次修复（P1/P2/P5/P6） | spec 第 4、5a 章 + 本手册附录 |
| 领域手册（沉淀） | `docs/domains/shop-style-theme.md` |