...
### Part A/B 线上实测完成状态（2026-09-19）✅

分层矩阵 A1-A5 与端到端 B1-B5 全部线上实测通过（详见测试手册，含手机视口截图）：

| 测试 | 断言 | 结果 |
|---|---|---|
| A1 L0 | 空配置 → 橙 `#FF6B00` + classic + 全块可见 | ✅ |
| A2/A3 L1/L2 | L1 页面级生效 + L2 覆盖 L1 + 停用回退 L1 | ✅ |
| A4/A5 L3/L4 | detailConfig 覆盖模板 + 单块缺省回退内建默认 | ✅ |
| B1 换肤 | 模板 palette 改色 → 全店联动 | ✅ |
| B2 切模板 | templateId 切换 → 全站变绿 | ✅ |
| B3 单店覆盖 | dualBuy + promo 隐藏 | ✅ |
| B4 停用回退 | 停用/删除模板 → 回退 L1 | ✅ |
| B5 新店默认 | 未引用模板 → L1 开箱即用 | ✅ |

### B 层实测新增发现（补入问题清单）

| # | 发现 | 说明 | 处理 |
|---|---|---|---|
| P5 | **改配置 C 端首帧不更新（缓存滞后）** | B2/B3 首次加载仍是旧配置，须冷加载（换 `?cb=` 二次 open）才生效 | 文档提示：改模板/渠道后强刷确认；判定以冷加载为准 |
| P6 | **引用模板时改 L1 全局 `themeTokens` 主色不生效** | `mergeThemeTokens` 主题令牌优先级 L1 < L2 模板 palette——**L1 主色被模板覆盖是设计语义，非 bug** | 文档标注：引用模板的店「全店换肤」真实入口 = 编辑模板 `palette.tokens`；未引用模板渠道才看 L1 主色 |

## 5a. 后台运营速览（测试后定稿要点）

1. **换肤入口**：店铺引用模板 → 改**模板 theme.palette**（L2）；不引用模板 → 改**全局配置 themeTokens**（L1）。
2. **改配置后**：C 端需刷新/冷加载，勿用首帧误判。
3. **单店微调**：渠道「店铺覆盖」detailConfig 只写想改的字段（数组/标量覆盖、null 跳过），其余回退模板/全局。
4. **模板异常**：停用/删除/跨端引用模板 → C 端自动回退 L1，不空白。
