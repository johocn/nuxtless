...
- 结算页行为（JD 版式与旧版式 `submitLegacy` 一致）：
  - `needAddress === true` → 显示「配送至」地址块，下单时调用 `submitAddress` 并校验（国家/省/市/区 + 收货人必填；街道可空），写入 `Order.shippingAddress`，供需地址的物流箱使用。
...
### 3.3 地址块布局（高德定位反查 + 级联回退）

地址块（`AddressBlock`，`needAddress` 时显示）：

- **主路径 · 自动反查**：进入结算页 / 地址块时，用当前定位调用**高德逆地理（regeocode）**，反查并自动填写 国家 / 省 / 市 / 区 / 街道，一行紧凑排开（窄屏可横滚）。
- **回退 · 定位/反查失败**：退化为「国家—省—市—区 级联下拉 + 街道 次级联下拉/手输」，并以**首页已定位城市兜底预填**，用户可在下拉中修正。
- **第二行**：街道（详细地址）承接门牌 —— 高德反查时自动填入、也可手动/下拉，**可为空**（非必填）；与「收货人 / 手机号」合理排布（收货人 + 手机号为基础信息行）。
- **必填**：物流配送档案（`needAddress === true`）时 国家 / 省 / 市 / 区 必填，缺任意一项阻止下单并 toast 提示；街道可为空（与 §3.2 一致）。
- **数据映射**：国家 → `countryCode`（默认 CN），省 → `province`，市 → `city`；高德返回的「区/县」「街道」在 Vendure 地址模型无独立字段，落库映射（置于 `city` / `streetLine1` 前段）在实现计划阶段与后端确认；街道落 `streetLine1`（可空）。前台始终以「自动反查，失败则级联 + 首页城市兜底」呈现。
...
- 地址块布局：进入自动高德逆地理反查并填 国家/省/市/区/街道；定位失败回退级联下拉且以「首页定位城市」兜底预填；物流档案 国家/省/市/区 必填、街道可空，缺省市区阻止下单并提示。
