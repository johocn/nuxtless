...
地址块（`AddressBlock`，`needAddress` 时显示）字段排布：

- **第一行**：国家 / 省 / 市 / 区 —— 四个**级联下拉**（先选省联动市、再联动区/县，国家 常用单值默认），**一行排开**（紧凑布局，窄屏可水平滚动）。
- **第二行**：**街道（详细地址）**输入框，承接门牌详细地址（`streetLine`），可与「收货人 / 手机号」合理排布（收货人 + 手机号视为基础信息行）。
- **必填**：配送物流档案（`needAddress === true`）时，国家 / 省 / 市 / 区 / 街道详细地址**全部必填**，缺任意一项阻止下单并通过 toast 提示（与 §3.2 的 `submitAddress` 校验一致）。
- **数据映射**：国家 → `countryCode`，省 → `province`，市 → `city`；「区 / 县」四级行政区划在 Vendure 地址模型无独立字段，落库映射（置于 `city` 或 `streetLine1` 前段）在实现计划阶段与后端确认后最终确定；前台交互始终以「国家—省—市—区 联动一行」呈现。
...
- 地址块布局：国家/省/市/区 在一行且级联联动，街道详细地址在独立一行；物流档案必填、缺失任意项阻止下单并提示。
