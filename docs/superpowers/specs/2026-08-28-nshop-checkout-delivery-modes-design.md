...
- 改造 `DeliveryModeBlock.vue` 为**单渲染器多版式**：
  - `folded`（默认 A）：四个入口平级选中，物流配送为「单入口」，选中后**内层展开**动态物流方式列表子项。
...
配送方式
├─ ● 物流配送            ← 单入口；选中后下方展开子项
│     ○ 邮寄   ● 京东物流  ○ 大件专送 …   （eligibleShippingMethods 动态单选）
├─ ○ 门店自提             ← 仅当 getPickupLocations('store') 非空显示
├─ ○ 职工单位自提          ← 仅当 employee 非空显示
└─ ○ 自提点               ← 仅当 point 非空显示
```

- 默认 mode 仍为 `shipping`，进入即展开物流子项并预选第一条。
- 切到某自提类型：物流子项收起，联动 `PickupBlock` 展示对应自提点列表，隐藏地址块。
...
- 本地 `pnpm dev` + agent-browser 验证：
  - `folded` 默认版式：四入口渲染、物流单入口展开子项、切自提收起子项、按数据隐藏入口（清空某类型自提点后该入口消失）。
