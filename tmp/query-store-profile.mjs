#!/usr/bin/env node
/** 聚焦查询：门店自提配送档案(id=1)的自提点绑定现状 + 全部自提点/档案/渠道 */
...
await q("profile_pickup join rows", `SELECT * FROM shipping_profile_pickup_locations_pickup_location ORDER BY "shippingProfileId"`);
...
await q("shipping_method all", `SELECT id, code, "customFields" FROM shipping_method ORDER BY id`);
await client.end();
console.log("\nDONE");
